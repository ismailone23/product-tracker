            ...
            {products && products.filter(data => data.product_name.toLowerCase().includes(searchText.toLowerCase())).map((stock, i) =>
                <div className='flex w-fulll lg:gap-2 gap-1' key={i}>
                    <div className='lg:w-32 w-28 h-20 lg:h-24 flex'>
                        {stock.image === 'no image' ?
                            <Image priority src={'/static/nophoto.jpeg'} className='w-full h-full object-cover rounded' alt="product image" width="0"
                                ...
                                sizes="100vw" />
                            :
                            <Image priority src={stock.image as string} className='w-full h-full object-contain rounded' alt="product image" width="0"
                                height="0"
                                sizes="100vw" />
                        }
                    </div>
                    <div className='flex flex-col'>
                        ...
                        <p className='lg:text-sm text-xs font-light md:font-normal'>{Number(stock.pricetable?.discount) > 0 && 'Regular Price '} ৳ {stock.price}</p>
                        {
                            stock.pricetable && (stock.pricetable.discount > 0 && <p className='lg:text-sm text-xs'>Discound price {
                        ...
                        <div className='flex lg:gap-4 gap-1'>
                            <button title='Clear this stock' className='cursor-pointer' onClick={() => handleIncDec('del', stock.id, stock)}>
                                {loading ? '...' : <TrashIcon className='xl:w-6 lg:w-5 w-4 text-red-600' />}</button>
                            <button title='Edit' onClick={() => { setIsUpdateOpen(!isUpdateOpen); setId(stock.id) }} className='cursor-pointer'>
                                {loading ? '...' : <PencilIcon className='xl:w-5 lg:w-4 w-3 text-blue-500' />}</button>
                        </div>
                    </div>
                </div>
            )}
